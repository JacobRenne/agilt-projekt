      ...
      <div class="modal-content bg-dark">
        <div class="modal-header">
          <h1 class="modal-title fs-6 text-white m-auto" id="confirmModalLabel">
            Är du säker att du vill ta bort allt i din varukorg?
          </h1>
          <button type="button" class="btn-close bg-light" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-footer d-flex justify-content-center">
          <button type="button" class="btn btn-primary" data-bs-dismiss="modal" @click="removeAllCartItems">Ja</button>
      ...
      <table class="table table-dark mb-4 rounded-3 overflow-hidden">
        ...
        <tbody class="table-group-divider">
          <tr v-for="(item, index) in cartItems" :key="item.id" class="">
            ...
            <td class="align-middle border-0">
              <button class="btn btn-danger border-0" @click="removeFromCart(index)">Ta bort</button>
            </td>
          </tr>
        </tbody>
      </table>
      <h4 class="col-6 d-flex me-auto ms-2">
        Totalt pris: <span class="text-info"> {{ totalPrice }} kr</span>
      </h4>
      <div class="col-12 d-flex justify-content-end gap-3 flex-wrap">
        <button class="btn btn-danger d-flex justify-content-center border-0" @click="confirmCancel">
          Töm varukorg
        </button>
        <RouterLink to="/thanks" class="text-decoration-none">
          <button class="btn btn-primary d-flex justify-content-center border-0 me-4" @click="sendToOrders">
            Slutför köp
          </button>
        </RouterLink>
      </div>
